...
export default class Reel extends cc.Component {
  ...
  changeCallback(element: cc.Node = null): void {
    ...
    if (el.position.y * dirModifier > 288) {
      ...
      if (pop != null && pop >= 0) {
        if(this.texture == pop){
          el.getComponent('Tile').startGlow(); //Começa a piscar os tiles escolhidos.
        }
  ...
  getTextureLenght(){ //Pega a quantidade de textura do tile 0.
    return this.tiles[0].getComponent('Tile').getTextureLenght();
  }

  getTextureChosen(texture){ //Pega a textura escolhida do pattern.
    this.texture = texture;
  }

  getTilesLenght(){ //Pega a quantidade de Tiles.
